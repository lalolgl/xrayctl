...
<p align="center">
  <em>Управление подписками, выбор профилей, автоматическая генерация конфигураций и запуск Xray с системной маршрутизацией через TUN.</em>
</p>
...
Другие протоколы могут работать при наличии соответствующей конфигурации Xray, но официально протестированными считаются только указанные выше.
...
7. 🔌 Предоставляет локальный SOCKS5-прокси
...
* определяет маршрут до удалённого Xray-сервера;
* сохраняет маршрут до сервера через исходный сетевой интерфейс;
* настраивает адрес TUN-интерфейса;
* заменяет системный маршрут по умолчанию маршрутом через TUN.

Это позволяет направлять системный трафик через Xray без необходимости отдельно настраивать каждое приложение.

> ⚠️ TUN-маршрутизация изменяет системный маршрут по умолчанию. Перед использованием убедитесь, что `xrayctl` имеет необходимые права для изменения сетевой конфигурации.

---

## 🔌 SOCKS5

Xray также предоставляет локальный SOCKS5-прокси:
...
* [x] Поддержка HWID
* [x] Просмотр и выбор профилей
* [x] Генерация конфигурации Xray
* [x] Проверка конфигурации перед запуском
* [x] Hysteria 2
* [x] VLESS
* [x] TUN-маршрутизация
* [x] Локальный SOCKS5-прокси
* [x] Запуск / остановка / статус Xray
* [x] Проверка состояния процесса Xray
* [x] Сохранение логов
...
* [ ] Корректное завершение работы и очистка маршрутов
